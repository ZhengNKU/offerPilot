...
class KnowledgeQuestionItem(BaseModel):
    title: str = ""
    freq: int = 0
    aiAnswer: AIAnswerSchema = Field(default_factory=AIAnswerSchema)
...
async def get_knowledge_questions(
    ...
    from app.services.question_generator import get_questions_for_sub_ability

    questions = await get_questions_for_sub_ability(
        db, user.id, body.sub_ability_name, redis_client,
    )
    return KnowledgeQuestionsResponse(questions=questions)
