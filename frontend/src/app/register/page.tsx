...
## 三、 数据保留与自动清理机制
我们深知您的个人数据不应在我们的系统中无限期留存，因此对您上传的文件、面试音频、简历分析与对话记录实施按会员等级的差异化保留策略，并由后台任务定期自动清理过期数据：

1. **保留期限**（自该文件最近一次上传时间起算）：
   - 面试音频、简历文件：内测用户保留 **30 天**。
   - 体验反馈截图：仅在您主动删除对应反馈时同步清除，不受 30 天自动清理约束。
2. **定期清理任务**：系统在后台持续运行清理任务，**默认每 24 小时扫描一次**，对超过保留期限的面试音频、简历文件从云端对象存储中删除对应的文件对象；为保留您历史分析报告、面试记录与反馈的完整引用，数据库中的文件记录不会被同步删除。

## 四、 您的权利与主动删除
您对您的个人数据拥有绝对的控制权：

1. **查看与修改**：您可以在"职业驾驶舱 - 账号与安全"中，随时查看并修改您的基本职业档案。
2. **主动删除单条记录**：在"模拟面试""简历分析""用户反馈"等页面，点击对应条目的删除按钮即可立即从云端与数据库中清除。
3. **注销账号**：在"职业驾驶舱 - 账号与安全"中点击"注销账号"，账号本身及其在数据库中的所有关联记录将立即、不可逆地清除；云端对象存储中的残留文件将在最近一轮定期清理任务（不超过 24 小时）内兜底删除。`;
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <p className="text-white/45 text-xs">更新日期：2026年7月23日</p>
              </div>
