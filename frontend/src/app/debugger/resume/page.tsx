...
function ResumeAnalysisPageContent() {
  ...
  const getDynamicBulletExample = (sectionIdx: number) => {
    ...
    const SEARCH_KEYWORDS: string[][] = [
      ...
      ["Mentor", "带人", "规范", "指导", "统筹", "主导", "团队", "协作", "配合", "对接", "沟通", "社团", "组织"]
    ];
    ...
    const FALLBACKS = [
      ...
      // 6 管理/协作经验
      {
        ...
        after: "作为团队骨干/小组负责人，牵头跨职能协作与任务拆解，关键项目交付准时率显著提升。"
      }
    ];
  ...
  const legacyGetDynamicSectionStatus = (sectionIdx: number) => {
    ...
    switch (sectionIdx) {
      ...
      case 6:
        // 管理/协作经验
        const mgtRisk = hasRisk(["管理", "带人", "导师", "mentor", "统统", "团队", "规模", "协作", "沟通"]);
        return {
  ...
  const legacyGetDynamicSectionDetails = (idx: number) => {
    ...
    const SECTIONS_INFO = [
      ...
      },
      {
        name: "管理/协作经验",
        desc: ds.status === "缺失"
          ? `对于目标定位是‘${targetRole}’的求职者，简历缺乏带人（Mentor）、技术规范主导、项目统筹以及跨团队协作影响力的描述。`
          : "简历中体现了技术指导、Mentor、工程规范落地或跨职能协同的骨干职责，具备团队影响力支撑。",
        advice: [
          "无需正式主管头衔：带新人（Mentor）、项目统筹、规范落地、跨组对接推动均可作为本段素材；应届生可写课题组分工、社团/竞赛队伍的组织与协同经历。",
          "突出跨团队的协同方式与影响力半径（对接了几个角色、推动了什么决策）。"
        ]
      }
    ];
